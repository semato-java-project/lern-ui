...
import {createItem, getList, requestTypes, updateItem} from "../../../services/httpService";
...
const ProjectGroup = ({group, isProjectDisabled, setIsProjectDisabled, lecturer = false}) => {
    ...
    const addProjectGroupTrigger = () => {
        dispatch(createItem(requestTypes.ADD_PROJECT_GROUP(urlParams.id)))
            .then(() => dispatch(getList(requestTypes.GET_PROJECT_GROUPS(urlParams.id))))
    };

    const joinToGroupTrigger = id => {
        dispatch(updateItem(requestTypes.EDIT_PROJECT_GROUP(id)))
            .then(() => dispatch(getList(requestTypes.GET_PROJECT_GROUPS(urlParams.id))))
    };
